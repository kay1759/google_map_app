...
export default (state = initialState, action: TAssociative) => {
	...
	case LOCATION.FILTERING_RESOURCE:
		...
		resources = state.get('resourcesAll').filter(function(loc: TAssociative) {
		    return (loc.locTypeId == action.categoryId);
		});
	    }

	    return state.update("categoryId", () => {return action.categoryId;})
		    .update("resources", () => {return resources;})
		    .update("resource", () => {return {id: 0, locTypeId: 0, title: "", longitude: "0.0", latitude: "0.0", image: "", address: "" };});
